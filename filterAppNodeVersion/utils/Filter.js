...
static simpleFilter(buffer, width, height, filter){
  ...
  let pixelOffset = (filter.length - 1)/2;

  //first and last few rows mus be skipped
  for(let i =pixelOffset; i < height-pixelOffset; i++){
  //last and first columns also need to be skipped
    for(let j = pixelOffset*4; j < (width*4)-(pixelOffset*4); j++){
      ...
      for(let k = 0; k < filter.length; k++){
        for(let l = 0; l < filter[0].length; l++){
          ...
          const componentAkl= data[((i +k -pixelOffset) * width*4 ) + j + (l -pixelOffset)*4];
          if(componentAkl){
      ...
      let result = parseInt(sum / divisor);
